...
if [ "$update_url" != "" ]; then
	...
	echo "---- [LiveUpdate] wget fw nvram webs_state_url ----"
	...
	echo "---- [LiveUpdate] wget fw MR1 ${dl_path_MR}1/$firmware_file ----"
...
if [ "$wget_result" != "0" ]; then
	...
	if [ "$firmware_check_ret" == "1" ] && [ "$rsasign_check_ret" == "1" ]; then
		...
		echo "---- [LiveUpdate] fw check error ----"
		rm -f $firmware_path
